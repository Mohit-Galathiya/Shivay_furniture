...
<table>
    ...
    <tr>
        <td><image height='40%' width='40%' src="../img/products/<?php echo $imgname ?>" /></td>
        <td><?php echo $name; ?></td>
        <td><?php echo $desc; ?></td>
        <td><?php echo $price; ?></td>
    </tr>
</table><br><br>
<form action="guest_product_crud.php?name=<?php echo $name; ?>" enctype="multipart/form-data" method="POST">
    Change the image:
...
if(isset($_POST['up_btn'])){
    ...
    if (isUploadedFileAnImage('img')) {
        ...
        $q = "UPDATE guest SET imgname='{$file}',price='{$price}',description='{$desc}' WHERE name='$name'";
        ...
        if (mysqli_query($conn, $q)) {
            ...
            if (move_uploaded_file($_FILES['img']['tmp_name'], $targetPath)) {
                ...
                ?>
                <script>
                    window.location.href="guest_product_crud.php?name='<?php echo $name; ?>'";
                </script>
                <?php
            } else {
        ...
        $q = "UPDATE guest SET price='{$price}',description='{$desc}' WHERE name='$name';";
        echo $q;
        if(mysqli_query($conn,$q)) {

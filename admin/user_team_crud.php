...
if(isset($_POST['up_btn'])){
    ...
    if (isUploadedFileAnImage('img')) {
        ...
        $q = "UPDATE user_dynamic SET imgname='{$file}',rate='{$rate}',description='{$desc}' WHERE name='$name'";
        ...
        if (mysqli_query($conn, $q)) {
            ...
            if (move_uploaded_file($_FILES['img']['tmp_name'], $targetPath)) {
                ...
                ?>
                <script>
                    window.location.href="user_team_crud.php?name='<?php echo $name; ?>'";
                </script>
                <?php
            } else {
        ...
        $q = "UPDATE user_dynamic SET rate='{$rate}',description='{$desc}' WHERE name='$name';";
        echo $q;
        if(mysqli_query($conn,$q)) {

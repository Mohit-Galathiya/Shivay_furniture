...
if(isset($_POST['up_btn'])){
    ...
    if (isUploadedFileAnImage('img')) {
        ...
        $q = "UPDATE guest SET imgname='{$file}',name='{$name}',description='{$desc}' WHERE role='mainimg'";
        ...
        if (mysqli_query($conn, $q)) {
            ...
            if (move_uploaded_file($_FILES['img']['tmp_name'], $targetPath)) {
                ...
                ?>
                <script>
                    window.location.href="guest_crud.php";
                </script>
                <?php
            } else {
        ...
        $q = "UPDATE guest SET name='{$name}',description='{$desc}' WHERE role='mainimg';";
        echo $q;
        if(mysqli_query($conn,$q)) {
